...
- `MatrixSparse` for matrices with sparse data. Optimize operations by not doing unnecessary additions and multiplications when empty cells are involved.
...
The underlying implementation should be unnoticed to the user since they are all created from the `Matrix` companion object. Then, the most optimized type possible is conserved through the transformation. When impossible to know the structure of the new matrix, the fallback is `MatrixDense`.


### Views

Some operations like transpose and many others do not need to actually change the matrice upon which they are realized. They could just operate on a view of the `RegFile`. This view is also a no-cost abstraction since it does not exist after meta-expansion. 
...
A "mini" particle Filter has been developed at first. The model has been simplified. It supposes that the drone always has the same normal orientation and only move in 2D, on the x and y axis. The state to estimate is only the 2D position. It is a plain particle filter and therefore, not conditioned on a latent variable and without any Kalman filtering. Thus, no matrix operations need to be applied. The sensors measurement are stored and loaded directly as constant values in the DRAM. This filter is a sanity check that the particle filter structure is sound, fittable on a Zynq board  and working as expected.
...
The RBPF implementation on hardware follows the expected structure of the filter thanks to Spatial high level of abstraction. To implement the sensors needing to be processed in order, one FIFO is assigned for each sensor. Then an FSM dequeue and update the filter one measurement at a time. The dequeued FIFO is the one containing the measurement with the oldest timestamp. This ensures that measurements are always processed one at a time and in order of creation's timestamp. 
...
The Rao-Blackwellized Particle Filter is a complex application. It would have been impractical, almost to the point of unfeasibility to attempt to build it with a reasonnable latency and throughput, in a timely manner, for a single person, if not for Spatial. We also gained insights about the development of complex applications for spatial and developed a new Matrix module as part of the standard library that might ease the development of new spatial applications.
